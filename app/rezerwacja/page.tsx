...
export default async function HomePage({ searchParams }: HomePageProps) {
  ...
  async function createPublicReservationAction(formData: FormData) {
    ...
    if (!appBaseUrl) {
      redirect(
        `/rezerwacja?${homeMonthQuery}&status=error&message=${encodeURIComponent("Brakuje adresu aplikacji do przekierowania na platnosc.")}`,
      );
    }
    ...
    try {
      ...
      redirect(`/rezerwacja?${homeMonthQuery}&status=error&message=${encodeURIComponent(errorMessage)}`);
    }
    ...
    <main className="page-shell">
      ...
      <section className="info-card booking-card">
        ...
        <div className="calendar-toolbar">
          <Link className="calendar-nav-button" href={`/rezerwacja?month=${monthCalendar.previousMonthParam}`}>
            Poprzedni miesiac
          </Link>
          <span className="calendar-toolbar-label">{monthCalendar.monthLabel}</span>
          <Link className="calendar-nav-button" href={`/rezerwacja?month=${monthCalendar.nextMonthParam}`}>
            Nastepny miesiac
          </Link>
        </div>
